...
const postLogin = async(req,res,next)=>{
    ...
    try{
        ...
        if (userFromDb.rowCount===0) return next(new ApiError(invalid_credentials))

        const user = userFromDb[0]
        if(!await compare(req.body.password,user.password)) return next(new ApiError(invalid_credentials,401))
